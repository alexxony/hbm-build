"""HBM2E 8-Hi 스택 균질화 유효 열전도율·체적 열용량 계산 핵심 모듈.

레이어 내부에 서로 다른 물질(예: Si 매트릭스 + Cu TSV)이 섞여 있을 때,
수직(z, 스택 적층 방향) 및 면내(xy, 다이 평면 방향) 유효 열전도율과
체적 열용량(ρ·cp, 방향성 없음)을 균질화 모델로 근사한다.
"""
import math
...
def volumetric_heat_capacity_mixing(
    fractions: dict[str, float], rho_cp_values: dict[str, float]
) -> float:
    """복합 레이어의 유효 체적 열용량 ρ·cp = Σ f_i · (ρ·cp)_i.

    체적 열용량(J/m³·K)은 전도와 달리 방향성이 없으므로(스칼라) k_z_mixing()과
    달리 텐서(k_xy/k_z) 분리가 불필요하며, 부피분율 가중 산술평균 하나로
    충분하다(열저항 관점의 직렬/병렬 구분이 체적 열용량에는 적용되지 않음 —
    단위 부피당 저장 에너지는 경로 무관하게 단순 가산).

    Args:
        fractions: 물질명 -> 부피분율. 합이 1 ± 1e-6이어야 함.
        rho_cp_values: 물질명 -> 체적 열용량 ρ·cp (J/m³·K). fractions와 동일한
            키 집합. ρ(kg/m³)·cp(J/kg·K)를 미리 곱해 전달한다.
    ...
    return sum(f * rho_cp_values[name] for name, f in fractions.items())
...
def _tsv_die_layer(name: str, thickness_um: float) -> dict:
    ...
    rho_cp = volumetric_heat_capacity_mixing(fractions, rho_cp_values)
    return {
        "name": name,
        ...
        "근거": (
            ...
            "rho_cp=부피분율 가중 산술평균(Cu f≈0.0103 + Si 나머지)"
        ),
    }
...
def _bump_layer(name: str, thermal_resistance_mm2k_w: float | None = None) -> dict:
    ...
    # rho_cp는 계면 열저항 override와 무관하게 항상 지오메트리 기반 부피분율로
    # 계산한다 — override는 k_z(전도 경로)만 대체하며, 체적 열용량은 저장된
    # 물질의 양(부피×ρ·cp)에만 의존하므로 본딩 방식이 바뀌어도 solder/underfill
    # 부피분율 자체는 변하지 않는다는 가정을 따른다.
    ...
    if thermal_resistance_mm2k_w is None:
        ...
        근거 = (
            ...
            "rho_cp=부피분율 가중 산술평균(solder f≈0.1623 + underfill 나머지, "
            "계면 열저항 override와 무관하게 지오메트리 부피분율 고정)"
        )
    else:
        ...
        근거 = (
            ...
            "근거 문서화, 정량 검증 아님). rho_cp도 지오메트리 부피분율 기반 유지 "
            "(본딩 방식이 solder/underfill 부피분율 자체를 바꾸지 않는다는 가정)."
        )
...
def layer_stack_hbm2e(
    ...
    Returns:
        레이어별 dict 목록. 각 dict: {name, thickness_um, k_xy, k_z, rho_cp, 근거}.
    ...
    layers.append(
        {
            ...
            "k_z": K_SI,
            "rho_cp": RHO_SI * CP_SI,
            "근거": "최상단 다이는 TSV 없음 → 순수 Si 벌크값 사용(rho_cp 포함)",
        }
    )
    ...
    layers.append(
        {
            ...
            "k_xy": K_EMC,
            "k_z": K_EMC,
            "rho_cp": RHO_EMC * CP_EMC,
            "근거": "몰딩 컴파운드, 등방성 벌크값 사용(rho_cp 포함)",
        }
    )
