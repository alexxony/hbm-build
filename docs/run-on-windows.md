...
### 5.0.3 "UnicodeEncodeError: 'cp949' codec can't encode character '≈'" — Windows 콘솔 출력 함정 (P3 T4 실측)

**증상**: `build_icepak_model.py`(또는 시나리오 스윕 스크립트)가 콘솔에 비교
결과를 출력하는 중 `print()`가 `UnicodeEncodeError`로 죽는다. 원인 문자는
`≈`(근사 기호, U+2248) 등 cp949(Windows 한국어 로케일 기본 콘솔 코드페이지)에
없는 유니코드 문자 — 스크립트 로직과 무관하게 **Windows 콘솔 인코딩
문제**다(Icepak/AEDT 해석 자체는 정상 완료됨, 출력 단계에서만 실패).

**해결**: 실행 전 환경변수로 stdout/stderr를 UTF-8로 강제한다.

```powershell
$env:PYTHONIOENCODING = "utf-8"
python scripts\build_icepak_model.py --power-scenario s1_phy_moderate ...
```

또는 세션 전체에 적용하려면 PowerShell 프로필에 추가. 근본 해결(스크립트
쪽 수정)이 필요하면 `≈`, `°` 등 비-ASCII 기호를 print 문에서 피하거나
`sys.stdout.reconfigure(encoding="utf-8")`를 스크립트 진입점에서 호출하는
방법도 있으나, 위 환경변수 우회가 가장 간단하고 코드 변경이 불필요하다.
